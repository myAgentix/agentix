...
class ChatDriver(Driver, Protocol):
    ...
    # adapters (OpenAIChatDriver, AnthropicChatDriver, GroqChatDriver and their
    # subclasses), not declared here — this Protocol is @runtime_checkable and a
    # required method would exclude gateway drivers (huble) and wrappers (failover
    # chain) that have no provider /models endpoint. Callers hasattr-guard it.
